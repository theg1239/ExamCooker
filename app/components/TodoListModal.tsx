  ...
  return (
    <>
      <button
        ...
        className={`font-bold py-2.5 px-2.5 ${isOpen ? 'bg-white/20 dark:bg-white/20' : ''} `}
      >
        <div>
          <img 
            src="/assets/Todo.svg" 
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          className="fixed bg-[#C2E6EC] dark:bg-[#0C1222] shadow-xl transform transition-all ease-in-out duration-300 opacity-100 z-50 border-2 border-[#5FC4E7] dark:border-[#008A90] overflow-hidden"
          ...
          <div className="p-4 overflow-y-auto" style={{ maxHeight: 'calc(70vh - 60px)' }}>
            <div className="flex mb-4">
              <input
                ...
                className="flex-grow border px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white dark:bg-[#3D414E]"
              />
              <button onClick={addTodo} className="bg-[#82BEE9] hover:bg-[#5FA0D9] dark:bg-[#008A90] text-white dark:text-[#D5D5D5] px-3 py-1 rounded-r transition duration-200">
                <PlusIcon size={20} />
              </button>
            </div>
